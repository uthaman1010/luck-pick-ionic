import {
    CreateAnimation, Gesture, GestureDetail, IonButton, IonButtons, IonContent, IonHeader,
    IonMenuButton, IonPage, IonTitle, IonToolbar, createGesture, useIonViewDidEnter
} from '@ionic/react';
...
const Tab2: React.FC = () => {
    ...
    useIonViewDidEnter(() => {
        ...
        const gesture: Gesture = createGesture({
            ...
            onStart: (event: any) => onStartHandler(event),
            onMove: (event: any) => onMoveHandler(event),
            onEnd: (event: any) => onMoveEnd(event)
        })
        gesture.enable()
    })
    ...
    const onMoveEnd = (detail: GestureDetail) => {
        elementRef.current!.style.transition = 'transform 500ms ease-out'
